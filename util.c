...
draw_box(char **output, node* cur, position pos) {
    ...
    /* draw the horizontal top and bottom of the box, unless it's an edge, in which case print a '.' to indicate a corner */
	for(int col = pos.x - NODE_W; col <= pos.x + NODE_W; col++) {
    	char c;

		if(col == pos.x - NODE_W || col == pos.x + NODE_W) c = CORNER;
		else c = HORIZONTAL;
...
draw_node(char **output, node* cur, int median, int mod, int depth) {
    ...
    /* if we have reached a leaf, just print the string below the last drawn box */
	if(cur->left == NULL && cur->right == NULL) {
	   	char leafstr[10];
	   	int strlen;
	   	strlen = sprintf(leafstr, "%s:%d", cur->str, cur->len);
	   	memcpy(&output[2+(depth*SPACING)][median - 2], leafstr, strlen);
	   	return;
	}
